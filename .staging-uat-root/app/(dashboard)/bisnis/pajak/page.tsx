...
import { Plus, Pencil, Trash2, X, Loader2, Percent, Star } from "lucide-react";
import type { TaxRate } from "@/lib/bisnis/types";
import { fetchTaxRates, createTaxRate, updateTaxRate, deleteTaxRate } from "@/lib/bisnis/client";

const EMPTY = { name: "", rate: 0, is_default: false };
...
export default function PajakPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await fetchTaxRates(false));
    } catch { /* */ }
    setLoading(false);
  }, []);
  ...
  const openEdit = (t: TaxRate) => {
    setEditId(t.id);
    setForm({ name: t.name, rate: t.rate, is_default: t.is_default });
    setShowModal(true);
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Hapus tarif pajak ini?")) return;
    try { await deleteTaxRate(id); load(); } catch { alert("Gagal menghapus"); }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Pajak / PPN</h1>
            <p className="mt-1 text-sm text-slate-500">Kelola tarif pajak untuk penjualan & pembelian</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
            <Percent className="mx-auto h-12 w-12 text-slate-200" />
            <h3 className="mt-4 text-lg font-semibold text-slate-700">Belum ada tarif pajak</h3>
            <p className="mt-1 text-sm text-slate-500">Buat tarif pajak pertama, misal PPN 11%</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Tarif (%)</th>
                  <th className="px-6 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Default</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {items.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-slate-700">{t.rate}%</td>
                    <td className="whitespace-nowrap px-6 py-4 text-center">
                      {t.is_default && <Star className="mx-auto h-4 w-4 text-amber-500 fill-amber-400" />}
                    </td>
                    <td className="whitespace-nowrap px-6 py-4 text-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-lg font-semibold text-slate-900">{editId ? "Edit Pajak" : "Tambah Pajak"}</h2>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              <div>
                ...
                <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="PPN 11%"
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-700">Tarif (%) <span className="text-red-500">*</span></label>
                <input required type="number" step="0.01" min="0" max="100" value={form.rate} onChange={(e) => setForm({ ...form, rate: parseFloat(e.target.value) || 0 })}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
