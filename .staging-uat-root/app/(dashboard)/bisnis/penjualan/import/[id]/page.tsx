...
import { ImportBatchAwbZipPanel } from "@/components/bisnis/ImportBatchAwbZipPanel";
import { ImportLineSerialFields } from "@/components/bisnis/ImportLineSerialFields";
import { fetchRequiresSerialMap } from "@/lib/wms/serial-numbers";
...
export default function ImportBatchDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setBatch(b);
      setLines(ln);
      const productIds = [...new Set(ln.map((l) => l.product).filter(Boolean) as string[])];
      setRequiresSerialMap(productIds.length > 0 ? await fetchRequiresSerialMap(productIds) : {});
    } catch (e: unknown) {
  ...
  const handleCancel = async () => {
    ...
    try {
      await cancelSalesImportBatch(batch.id);
      await load();
    } catch (e: unknown) {
  ...
  const activity = salesBatchToActivity(batch, lines);
  const canPost =
    batch.status !== "cancelled" &&
    activity.displayStatus !== "success" &&
    lines.some((l) => l.validation_status === "valid");
  const canCancel = activity.canCancel;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        ...
        >
          <ArrowLeft className="h-3.5 w-3.5" /> Riwayat import massal
        </Link>
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">
              {batch.expand?.store_channel_account?.account_name} · {fmtDate(batch.period_from)} –{" "}
              {fmtDate(batch.period_to)}
        ...
        {canPost && Object.values(requiresSerialMap).some(Boolean) ? (
          <p className="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
            Beberapa produk wajib SN — isi serial di tabel order sebelum posting.
          </p>
        ) : null}
        {canPost && (
        ...
        <div className="mb-6">
          <ImportBatchAwbZipPanel
            batchId={batch.id}
            batchPosted={activity.displayStatus === "success" || (batch.posted_rows ?? 0) > 0}
          />
        </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {g.lines.map((l) => (
                    <tr key={l.id} className={l.validation_status === "error" ? "bg-red-50/50" : ""}>
                      <td className="px-4 py-2 font-mono text-xs align-top">{l.mp_sku}</td>
                      <td className="align-top">
                        <div>{l.expand?.product?.name ?? l.product_name ?? "—"}</div>
                        {l.product && requiresSerialMap[l.product] && l.validation_status === "valid" ? (
                          <ImportLineSerialFields line={l} onSaved={() => void load()} />
                        ) : null}
                      </td>
                      ...
                      <td className="px-4 py-2">
                        ...
                        {l.validation_status === "posted" && l.invoice && (
                          <Link
                            href={`/bisnis/penjualan/${l.invoice}`}
...
function groupByOrder(lines: SalesImportLine[]) {
  const map = new Map<
    string,
    { orderNo: string; orderDate: string; lines: SalesImportLine[]; gross: number; fees: number; net: number }
  >();
