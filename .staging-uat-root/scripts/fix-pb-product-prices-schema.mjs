/**
 * Harga jual per toko — field store di biz_product_prices.
 * Run: npm run pb:product-prices-schema
 */
import fs from "fs";
...
async function patchCollection(name, fields) {
  ...
  if (!changed) {
    console.log(`Schema ${name} (product prices) sudah benar.`);
    return;
  }
...
await patchCollection("biz_product_prices", [
  {
    system: false,
    id: fieldId("pprst"),
    name: "store",
...
console.log("Selesai — schema harga per toko siap.");
