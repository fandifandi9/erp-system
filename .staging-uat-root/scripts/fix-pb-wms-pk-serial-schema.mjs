/**
 * PK number + serial number fields untuk WMS outbound.
 * Run: node scripts/fix-pb-wms-pk-serial-schema.mjs
 */
import fs from "fs";
...
function ensureField(schema, field) {
  ...
  if (schema[idx].type !== field.type) {
    console.warn(`SKIP ${field.name}: sudah ada sebagai ${schema[idx].type}`);
    return false;
  }
  return false;
}
...
await patchCollection("biz_sales_orders", [textField("pk_no", "pkno")]);

await patchCollection("biz_sales_order_lines", [textField("serial_numbers_json", "snjson")]);

await patchCollection("inv_products", [boolField("requires_serial", "reqsn")]);

console.log("Selesai — PK + serial fields siap.");
