...
export default function MyRatingTasksPage() {
  ...
  useEffect(() => {
    void (async () => {
      const res = await fetch("/api/hr/rating/my-tasks", { headers: authHeaders() });
  ...
  async function openTask(id: string) {
    setActiveId(id);
    setMsg(null);
    const res = await fetch(`/api/hr/rating/tasks/${id}`, { headers: authHeaders() });
    const json = await res.json();
    if (!res.ok) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 p-6">
      <Link href="/dashboard-staff/rating" className="text-sm text-indigo-700 underline">
        ← Hasil rating
      </Link>
