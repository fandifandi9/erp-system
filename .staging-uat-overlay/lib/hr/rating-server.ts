      ...
      officeId:
        typeof p.office_id === "string"
          ...
          : p.office_id && typeof p.office_id === "object" && "id" in (p.office_id as object)
            ? String((p.office_id as { id: string }).id)
            : "",
    };
  } catch {
...
async function buildCandidateUniverse(
  ...
  for (const p of profiles) {
    const uid = String((p as { user?: string }).user || "");
    if (!uid) continue;
    const office = (p as { office_id?: string }).office_id;
    profileByUser.set(uid, {
      department: String((p as { department?: string }).department || ""),
      division: String((p as { division?: string }).division || ""),
  ...
  for (const u of users) {
    ...
    const accountType = String((u as { account_type?: string }).account_type || "");
    ...
    out.push({
      userId: id,
      status: String((u as { status?: string }).status || "active"),
...
async function previousPeriodReviewerIds(
  ...
  try {
    ...
    const prev = periods.find((p) => ymd((p as { end_date?: string }).end_date) < start) || periods[0];
    if (!prev) return [];
    ...
    for (const a of assignments) {
      ...
      for (const r of revs) {
        ids.push(String((r as { reviewer: string }).reviewer));
      }
    }
    return [...new Set(ids)];
  } catch {
...
export async function serverCreateAssignment(
  ...
  // HR cannot create assignment for themselves (Owner must)
  if (ctx.isHr && !ctx.isOwner && subjectId === ctx.userId) {
  ...
  for (const a of periodAssignments) {
    ...
    for (const r of revs) alreadyAssignedInPeriod.add(String((r as { reviewer: string }).reviewer));
  }
  ...
  for (const rid of selectedIds) {
    const tier =
      method === "smart_random"
        ? (evidence as { selected?: { userId: string; tier: string }[] }).selected?.find(
            (s) => s.userId === rid,
          )?.tier
        : "manual";
...
export async function serverGetReviewerTask(
  ...
  if (String(row.reviewer) !== ctx.userId && !ctx.isOwner && !ctx.isHr) {
    throw new HrApiError("Akses ditolak.", 403);
  }
  if (String(row.reviewer) !== ctx.userId && (ctx.isHr || ctx.isOwner)) {
    // HR/Owner may view — ok
  } else if (String(row.reviewer) !== ctx.userId) {
    throw new HrApiError("Akses ditolak.", 403);
  }
...
export async function serverSaveReviewerDraft(
  ...
  const row = (await adminPb.collection(RATING_COLLECTIONS.reviewers).getOne(reviewerRowId, {
    requestKey: null,
  })) as { id: string; reviewer: string; status: string; assignment: string };
...
export async function recalculateAssignmentResult(adminPb: PocketBase, assignmentId: string) {
  ...
  const done = all.filter((r) => ["submitted", "locked"].includes(String((r as { status: string }).status)));
...
export async function serverGetMyResult(
  ...
  if (String((assignment as { subject: string }).subject) !== ctx.userId) {
    throw new HrApiError("Akses ditolak.", 403);
  }
  ...
  if (!result) {
    return {
      ...
      period: (assignment as { expand?: { period?: unknown } }).expand?.period ?? null,
...
export async function serverGetAssignmentDetail(
  ...
  return {
    ...
    selection_evidence: safeParseJson(assignment.selection_evidence_json),
  };
}
...
export async function serverListAssignmentsForHr(
  ...
  if (ctx.isOwner) return list;
  const out = [];
  for (const a of list) {
    try {
      await assertSubjectInActorScope(adminPb, ctx, String((a as { subject: string }).subject));
      out.push(a);
    } catch {
      /* skip out of scope */
    }
