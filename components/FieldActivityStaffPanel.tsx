    ...
    <div className="mx-auto max-w-3xl space-y-8 p-6">
      ...
      <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="text-xs font-medium text-slate-700">Jenis aktivitas</label>
            <select
              value={activityType}
          ...
          <div className="sm:col-span-2">
            <label className="text-xs font-medium text-slate-700">Tujuan / lokasi singkat *</label>
            <input
              required
              minLength={2}
              value={destination}
              ...
              className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-600"
            />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            <label className="text-xs font-medium text-slate-700">Keterangan / agenda (min. 10 karakter) *</label>
            <textarea
              required
              rows={4}
              value={reason}
              ...
              className={`mt-1 w-full rounded-lg border px-3 py-2 text-sm text-slate-900 placeholder:text-slate-600 ${
            ...
            {rows.map((r) => (
              <div key={r.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center gap-2">
                  {badge(r)}
                  <span className="text-xs font-medium text-slate-700">{ACTIVITY_TYPE_LABEL[r.activity_type]}</span>
                </div>
