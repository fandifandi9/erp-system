...
import { findCustomerByExactName } from "@/lib/bisnis/customer-lookup";
import { buildQuickCustomerCode } from "@/lib/bisnis/customer-quick-add";
...
export default function CustomerPage() {
  ...
  const loadCustomers = useCallback(async (p: number, q: string, type: "" | CustomerType = "") => {
    try {
      ...
      const parts: string[] = [];
      if (q.trim()) {
        const qEsc = q.trim().replace(/\\/g, "\\\\").replace(/"/g, '\\"');
        parts.push(`(name ~ "${qEsc}" || phone ~ "${qEsc}")`);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (nameTaken && nameTaken.id !== editId) {
        alert(`Nama "${form.name.trim()}" sudah terdaftar. Gunakan nama lain (mis. dudung2 / dudung gendut).`);
        return;
      }
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {customers.length === 0 && (
                    <tr><td colSpan={7} className="px-5 py-12 text-center text-sm text-slate-400">Tidak ada pelanggan ditemukan.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <div>
                ...
                {!editId ? (
                  <p className="mt-0.5 text-xs text-slate-500">
                    Nama harus unik. &quot;dudung&quot; tidak boleh dobel; &quot;dudung2&quot; / &quot;dudung gendut&quot; boleh.
                  </p>
                ) : null}
              </div>
