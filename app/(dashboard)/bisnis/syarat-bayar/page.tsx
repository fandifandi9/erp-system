...
import type { PaymentCondition } from "@/lib/bisnis/types";
import { fetchPaymentConditions, createPaymentCondition, updatePaymentCondition, deletePaymentCondition } from "@/lib/bisnis/client";

const EMPTY = { name: "", description: "" };

export default function SyaratBayarPage() {
  const [items, setItems] = useState<PaymentCondition[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setItems(await fetchPaymentConditions(false)); } catch { /* */ }
    setLoading(false);
  }, []);
  ...
  const openEdit = (t: PaymentCondition) => {
    setEditId(t.id);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Hapus syarat pembayaran ini?")) return;
    try { await deletePaymentCondition(id); load(); } catch { alert("Gagal menghapus"); }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editId) await updatePaymentCondition(editId, payload);
      else await createPaymentCondition(payload);
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Syarat Pembayaran</h1>
            <p className="mt-1 text-sm text-slate-500">Kelola syarat & ketentuan pembayaran transaksi</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
            ...
            <h3 className="mt-4 text-lg font-semibold text-slate-700">Belum ada syarat pembayaran</h3>
            <p className="mt-1 text-sm text-slate-500">Contoh: &quot;COD&quot;, &quot;DP 50%&quot;, &quot;Full Payment&quot;</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Keterangan</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-lg font-semibold text-slate-900">{editId ? "Edit Syarat" : "Tambah Syarat"}</h2>
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              <div>
                ...
                <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Cash On Delivery"
              ...
              <div>
                ...
                <textarea rows={3} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                  placeholder="Deskripsi atau syarat tambahan..."
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
