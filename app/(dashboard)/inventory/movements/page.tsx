...
export default function InventoryMovementsPage() {
  ...
  const handleQuickPost = async (id: string) => {
    ...
    try {
      ...
      setBanner("Movement berhasil diposting — cek tab Stok.");
      await load();
    } catch (err) {
  ...
  return (
    <InventoryGate>
      <InventoryShell title="Stock movement" subtitle="Draft belum mengubah stok. Klik Post agar qty masuk dashboard Stok.">
        {banner ? (
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <Plus className="h-4 w-4" /> Movement baru
          </Link>
        </div>
