...
import { fetchCategories, saveCategory } from "@/lib/inventory/client";
import { canManageInventoryMaster } from "@/lib/inventory/access";
import { getErrorMessage } from "@/lib/errors";
import type { InvCategory } from "@/lib/inventory/types";
import { Loader2, Plus } from "lucide-react";

export default function InventoryCategoriesPage() {
  const user = pb.authStore.model;
  const canEdit = user && canManageInventoryMaster(user);
  const [items, setItems] = useState<InvCategory[]>([]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      setItems(await fetchCategories(false));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <InventoryGate>
      <InventoryShell title="Kategori produk" subtitle="Kategori untuk pengelompokan SKU.">
        {canEdit ? (
          <button
            type="button"
              ...
              {loading ? (
                ...
                items.map((c) => (
                  <tr key={c.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-3">{c.is_active !== false ? "Ya" : "Tidak"}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        {modal ? (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
            <form onSubmit={submit} className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
              <h3 className="text-lg font-semibold">Kategori baru</h3>
