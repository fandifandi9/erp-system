        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="text-sm">
            ...
            <select className="mt-1 w-full rounded border px-3 py-2" value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)}>
              <option value="">
                {periods.length === 0
                  ? t("hr.rating.assignments.noPeriodsPlaceholder")
                  : t("hr.rating.assignments.periodPlaceholder")}
              </option>
              {periods.map((p) => (
            ...
            {periods.length === 0 ? (
              <p className="mt-1 text-xs text-amber-800">
                {t("hr.rating.assignments.noPeriodsHint")}{" "}
                <Link href="/hr/rating/periods" className="font-medium underline">
                  {t("hr.rating.nav.periods")}
                </Link>
              </p>
            ) : null}
          </label>
