...
export default function PublicQuotationSharePage() {
  const { id } = useParams<{ id: string }>();
  const searchParams = useSearchParams();
  const showPreview = searchParams.get("view") === "1";

  const [data, setData] = useState<ShareQuotation | null>(null);
  ...
  const publicPath =
    typeof window !== "undefined"
      ? `${window.location.origin}/share/quotation/${id}`
      : `/share/quotation/${id}`;

  if (!showPreview) {
    return (
      <ShareOpenGate
        docLabel="Penawaran penjualan"
        docNo={data.order_no}
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10">
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 bg-amber-50/80 px-6 py-5">
        ...
        <div className="space-y-4 px-6 py-6 text-sm">
          ...
          <div className="flex justify-between gap-4 border-t border-slate-100 pt-3">
            <span className="text-slate-500">Total penawaran</span>
