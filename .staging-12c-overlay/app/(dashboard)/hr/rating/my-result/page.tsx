  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        setError(translateRatingApiError(e instanceof Error ? e.message : "", t, "hr.rating.myResult.loadError"));
      }
    })();
  }, [t]);

  if (error) return <div className="text-red-600">{error}</div>;
  if (data === undefined) return <div className="text-slate-500">{t("common.loading")}</div>;
      ...
      {!result ? (
        ...
        <>
          <div className="rounded border bg-white p-4">
            ...
            <p className="text-xs text-slate-500">{t("hr.rating.results.respondentsHelp")}</p>
            <p className="text-xs text-slate-400">
              {String(result.aggregate_kind) === "current"
                ? t("hr.rating.myResult.currentAggregate")
                : String(result.aggregate_kind) === "final"
                  ? t("hr.rating.myResult.finalAggregate")
                  : ""}
            </p>
          </div>
